...
import { Settings, Shield, Clock, Gem } from 'lucide-react';

import HeroSection from '../HeroSection';
import FeatureCard from '../FeatureCard';
import MetricsPanel from '../MetricsPanel';

const Step01 = ({ onNext, onPrev }) => {
  const { t } = useLanguage();
  const stepData = t('diamondJourney.step1');
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_290px] gap-6 mt-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <FeatureCard 
            icon={Settings} 
            title={stepData.card1Title} 
            body={stepData.card1Body} 
          />
          <FeatureCard 
            icon={Shield} 
            title={stepData.card2Title} 
            body={stepData.card2Body} 
          />
          <FeatureCard 
            icon={Clock} 
            title={stepData.card3Title} 
            body={stepData.card3Body} 
          />
          <FeatureCard 
            icon={Gem} 
            title={stepData.card4Title} 
            body={stepData.card4Body} 
          />
        </div>
